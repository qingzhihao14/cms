...
//			ClientGlobal.init("/home/briup/fdfs_client.conf");
	...
	public static String[] upload(byte[] local_filename,String ext_name) throws Exception {
		try {
			...
			System.out.println("storageServers?"+storageServers);
			for(int ii=0;ii<storageServers.length;ii++){
				System.out.println("第"+ii+"个storageServers="+storageServers[ii]);
			}
			...
			for(int jj=0;jj <result.length;jj++){
				System.out.println("第"+jj+"个result="+result[jj]);
			}
			return result;
		} catch (Exception e) {
			e.printStackTrace();
